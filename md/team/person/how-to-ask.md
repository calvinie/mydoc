...
在查看Stack Exchange之前，先从谷歌搜索开始；谷歌实时对其进行索引。很有可能有人已经问过类似的问题，Stack Exchange网站通常接近搜索结果的顶部。如果您没有通过谷歌找到任何东西，请在与您的问题最相关的特定网站上再次搜索（见下文）。使用标签搜索可以帮助缩小结果范围。

如果您仍然没有找到任何东西，请在最贴近主题的网站上发布您的问题。使用格式化工具，特别是代码，并添加与您的问题实质内容相关的标签（特别是您遇到问题的编程语言、操作系统或库的名称）。如果评论者要求您提供更多信息，请编辑您的主要帖子以将其包含。如果任何答案有帮助，请单击向上箭头进行投票；如果答案为您的问题提供了解决方案，请单击投票箭头下的勾号以接受其正确。

Stack Exchange已经发展到100多个网站，但以下是最有可能的候选网站：

- 超级用户是关于通用计算的问题。如果您的问题不是关于您仅通过网络连接与之交谈的代码或程序，它可能在这里。
- Stack Overflow用于有关编程的问题。
- 服务器故障适用于有关服务器和网络管理的问题。

几个项目都有自己的特定站点，包括Android、Ubuntu、TeX/LaTeX和SharePoint。检查Stack Exchange网站以获取最新列表。
...
如果你想在回复中提问，确保改变主题以表明你是在问一个问题，一个主题象 `Re: test` 或者 `Re: new bug` 的消息不太可能引起足够的注意。同时，将回复中与新主题不甚相关的引用内容尽量删除。
...
### 使问题容易回复
以请`向……回复`来结束问题多半会使你得不到回答。如果你觉得花几秒钟在邮件客户端设置一下回复地址都麻烦，我们也觉得花几秒钟考虑你的问题更麻烦。如果你的邮件客户端程序不支持这样做，换个好点的；如果是操作系统不支持所有这种邮件客户端程序，也换个好点的。
...
正确拼写、标点和大写。不要将“its”与“it's”混淆，“loose”与“lose”混淆，或将“discrete”与“discreet”混淆。不要全部输入大写字母；这被解读为大喊大叫，被认为是粗鲁的。（全部小写也好不到哪去，因为不易阅读。Alan Cox [注：著名黑客，Linux 内核的重要参与者] 也许可以这样做，但你不行。）

更一般地说，如果你像一个半识字的胸部一样写作，你很可能会被忽视。所以不要使用即时消息快捷方式。将“你”拼写为“u”会让你看起来像一个半文盲的胸部，以节省两个完整的按键。更糟糕的是：像l33t剧本一样写作，kiddie hax0r是绝对的死亡之吻，并保证你只会得到石质的沉默（或者充其量是一堆的蔑视和讽刺）作为回报。

如果您在不使用母语的论坛上提问，您将在拼写和语法错误方面得到有限的松懈——但懒惰完全没有额外的松懈（是的，我们通常可以发现这种差异）。此外，除非您知道受访者的语言是什么，否则请用英语书写。忙碌的黑客往往只是用他们不懂的语言来冲洗问题，而英语是互联网的工作语言。通过用英语写作，你最大限度地减少了你的问题被丢弃不读的机会。

如果您正在用英语写作，但它是您的第二语言，那么提醒潜在的受访者注意潜在的语言困难和绕过它们的选择是一种很好的形式。示例：

- 英语不是我的母语；请原谅打字错误。
- 如果您会说$LANGUAGE，请给我发电子邮件/PM；我可能需要帮助翻译我的问题。
- 我熟悉技术术语，但一些俚语表达和成语对我来说很难。
- 我已经用$LANGUAGE和英语发布了我的问题。如果您只使用其中一个，我很乐意翻译回复。
...
[西蒙.泰瑟姆（Simon Tatham）](https://www.chiark.greenend.org.uk/%7Esgtatham/) 写过一篇[《如何有效报告BUG》](https://www.chiark.greenend.org.uk/%7Esgtatham/bugs-cn.html)，我强烈推荐各位阅读。
...
# 别急于宣称找到BUG
当你在一个软件中遇到问题，除非你 非常、非常 的有根据，不要动辄声称找到了`BUG`。提示：除非你能提供解决问题的源代码补丁，或者对前一版本的回归测试表现出不正确的行为，否则你都多半不够完全确信。对于网页和文档也如此，如果你（声称）发现了文档的BUG，你应该能提供相应位置的替代文本。

记住，还有许多其它用户并未经历你遇到的问题，否则你在阅读文档或搜索网页时就应该发现了 (你在报怨前已经做了这些,[不是吗？](#提问前)）？。这也意味着很有可能是你弄错了而不是软件本身有问题。
...
“读读该死的手册”（RTFM）有个年轻一点的亲戚，如果你收到“搜搜该死的网络”（STFW）的回复，发信人认为你应该“搜搜该死的网络”。那人多半也是对的，去搜一下吧。(更温和一点的说法是“[谷歌是你的朋友！](www.google.com)
...
## 相关资源
如果需要个人电脑、Unix 和互联网如何工作的基础知识，参阅 [Unix 和互联网工作的基本原理](https://tldp.org/HOWTO/Unix-and-Internet-Fundamentals-HOWTO/)。
